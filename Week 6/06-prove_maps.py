...
class Translator:
    ...
    def translate(self, from_word):
        ...
        """
        try:
            translated = self.words[from_word]
        except:
            translated = "???" 
...
def summarize_degrees(filename):
    ...
    """
    degrees = {
        'Doctorate': 0, 'Masters': 0, 'Bachelors': 0, 'Assoc-acdm': 0,
        'Assoc-voc': 0, 'Prof-school': 0, 'Some-college': 0, 'HS-grad': 0,
        '12th': 0, '11th': 0, '10th': 0, '9th': 0, '7th-8th': 0, 
        '5th-6th': 0, '1st-4th': 0, 'Preschool': 0
    }
    with open(filename) as file_in:
        # Each 'line' is a row in the file
        for line in file_in:
            # 'fields' is a split-up version of each 'line'
            fields = line.split(",")
            # 'degree' is the level of education
            degree = fields[3]
            # Select the degree based on the file input, and record it
            try:
                degrees[degree] += 1
            except:
                pass
...
def is_anagram(word1, word2):
    ...
    using the [] notation.
    """
    # Put every letter in the first word into a stack. Then, iterate through
    # every letter in the second word and check it against the stack; 
    # when a match is found, eliminate it from the stack; if the stack 
    # still has letters in it by the end, then they are not anagrams
    stack = []
    # Put every alphabetical letter in first word into the stack
    for letter in word1:
        if letter.isalpha():
            stack.append(letter.lower())
    # Check every alphabetical letter in word2 against the stack to look
    # for matches
    for letter in word2:
        if letter.isalpha():
            if letter.lower() in stack:
                stack.remove(letter.lower())
            # If no match is found, word2 contains a letter
            # which word1 does not. Not an anagram
            else:
                return False
    
    # Test if stack is empty
    is_anagram = len(stack) == 0
    return is_anagram
...
class Maze:
    ...
    def get_walls(self):
        return self.maze_map[(self.curr_x, self.curr_y)]

    def display_impassable(self):
    ...
    def move_left(self):
        ...
        """
        # Test if left wall is open. If yes, decrement x
        walls = self.get_walls()
        if walls[0]:
            self.curr_x -= 1

        else:
            self.display_impassable()
    ...
    def move_right(self):
        ...
        can't move, then display "Can't go that way!"
        """
        # Test if right wall is open. If yes, increment x
        walls = self.get_walls()
        if walls[1]:
            self.curr_x += 1
        else:
    ...
    def move_up(self):
        ...
        can't move, then display "Can't go that way!"
        """
        # Test if lower wall is open. If yes, decrement y
        walls = self.get_walls()
        if walls[2]:
            self.curr_y -= 1
        else:
    ...
    def move_down(self):
        ...
        can't move, then display "Can't go that way!"
        """
        # Test if upper wall is open. If yes, increment y
        walls = self.get_walls()
        if walls[3]:
            self.curr_y += 1
        else:
...
def earthquake_daily_summary():
    ...
    # Select and print the proper data
    ...
    for feature in features:
        ...
        # Select magnitude
        mag = properties['mag']
        # Display place and magnitude of each earthquake
