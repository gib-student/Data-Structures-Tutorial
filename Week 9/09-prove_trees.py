...
class BST:
    ...
    def __reversed__(self):
        """
        Perform a forward traversal (in order traversal) starting from 
        the root of the BST.  This function is called when the 
    ...
    def _traverse_backward(self, node):
        ...
        """
        if node is not None:
            yield from self._traverse_backward(node.right) # for loop
            yield node.data # Start displaying after loop ends
...
# Start Problem 5 #
###################
def _insert_middle(sorted_list, first, last, bst, iterations=0):
    """
    ...
    """
    # iterations += 1
    # assert iterations <= 25, "Iterations > 25"

    # Condition: sorted_list is empty, or we've reached the end of eithe left or right. 
    # We want to quit
    length = last - first       # if list is empty or we've reached the end this should be -1
    if length < 0:
        return
    # Condition: the range has only one value. We want to insert the value
    # into the bst and quit
    elif length == 0:
        bst.insert(sorted_list[first])
        return
    elif length > 0:
        # Now we address the normal condition, which is that the range contains
        # multiple values, we want to find the middle and insert it,
        # and then do recursion to both sides.
        # Step 1: find the middle
        middle = (length // 2) + first      # the middle rounded down
        # Step 2: insert the middle value into the bst
        bst.insert(sorted_list[middle])
        # Step 3: Recursion to the left side. First will stay the same,
        # end will be one left of the middle
        _insert_middle(sorted_list, first, middle - 1, bst, iterations)
        # Step 3: Recursion to the right. First will be one right of the middle,
        # end will stay the same
        _insert_middle(sorted_list, middle + 1, last, bst, iterations)
        # Last step: quit
        return
